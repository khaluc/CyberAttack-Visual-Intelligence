...
function knowledgePage(){
 ...
 const cards=sources?order.map(key=>{const s=sources[key]||{},count=key==="enterprise_assets"?`${s.assets||0} assets`:`${s.documents||0} documents`,status=s.ready?"✓ Sẵn sàng":s.last_error?"× Có lỗi":"○ Chưa có dữ liệu";return `<button class="knowledge-card panel" onclick="filterKnowledge('${key}')"><div><span style="font-size:26px;color:#087f8c">◇</span><span class="synced ${s.ready?"":"inactive"}">${status}</span></div><h3>${esc(s.label||key)}</h3><p>${esc(descriptions[key])}</p><small><span>${count} · ${s.files||0} files</span><b>›</b></small>${s.last_error?`<em>${esc(shortText(s.last_error,120))}</em>`:""}</button>`}).join(""):`<div class="knowledge-loading panel">Đang đọc registry và SQLite index...</div>`;
